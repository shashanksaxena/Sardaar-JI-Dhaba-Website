...
import { ArrowRight, ArrowUpRight, ChevronLeft, Clock3, Instagram, MapPin, Menu as MenuIcon, MessageCircle, Phone, Search, Star, Utensils, X } from 'lucide-react';
...
function MenuPage() {
  ...
  return <Shell><Meta title="Sample menu" /><PageIntro eyebrow="The menu · editable sample" title="Come hungry. Leave happier." copy="A working sample of the menu we are shaping. Dish details, availability and pricing will be updated by the Sardaar JI team." /><section className="mx-auto max-w-7xl px-5 pb-20 lg:px-8 lg:pb-28"><div className="flex flex-col gap-5 border-y border-foreground/10 py-5 lg:flex-row lg:items-center lg:justify-between"><div className="flex gap-2 overflow-x-auto pb-1">{categories.map(category => <button type="button" key={category} onClick={() => setActive(category)} className={`whitespace-nowrap rounded-full border px-4 py-2 text-xs font-bold transition-colors ${active === category ? 'border-primary bg-primary text-primary-foreground' : 'border-foreground/15 hover:border-primary hover:text-primary'}`} data-testid={`button-filter-${category.toLowerCase()}`}>{category}</button>)}</div><label className="flex items-center gap-3 border-b border-foreground/20 pb-2 text-sm lg:w-60"><Search size={16} className="text-muted-foreground" /><span className="sr-only">Search menu</span><input type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search the menu" className="w-full bg-transparent outline-none placeholder:text-muted-foreground" data-testid="input-menu-search" /></label></div><div className="mt-12 grid gap-x-10 gap-y-8 md:grid-cols-2">{filtered.map(item => <MenuCard item={item} key={item.id} />)}</div>{filtered.length === 0 && <div className="py-20 text-center"><Utensils className="mx-auto text-primary" /><p className="mt-4 font-display text-2xl">That plate is not on today's list.</p><button type="button" onClick={() => { setQuery(''); setActive('All'); }} className="mt-4 text-sm font-bold text-primary underline" data-testid="button-reset-menu">Reset the menu</button></div>}<div className="mt-16 border border-dashed border-primary/50 bg-primary/5 p-5 text-sm leading-6 text-muted-foreground"><strong className="text-foreground">Menu note:</strong> This is an editable sample. Menu items, prices, dietary information and availability are placeholders pending final confirmation.</div></section></Shell>;
}
...
function Blog() {
  ...
  return <Shell><Meta title="Restaurant Style Indian Recipes" /><PageIntro eyebrow="Blog" title="Restaurant-style recipes from the dhaba kitchen." copy="Step-by-step guides for butter chicken, kadhai chicken, dal makhani, paneer tikka and more North Indian favourites." /><section className="mx-auto max-w-7xl px-5 pb-24 lg:px-8"><div className="grid gap-7 md:grid-cols-3">{visibleStories.map((story, i) => <Link href={`/blog/${story.id}`} key={story.id} className={`group ${i % 3 === 1 ? 'md:translate-y-12' : ''}`} data-testid={`card-blog-${story.id}`}><PhotoArt src={story.image} alt={story.alt} label={story.category} className="min-h-[270px]" /><p className="mt-6 font-mono-brand text-[10px] uppercase tracking-[.18em] text-primary">{story.publishedAt} · {story.category} · {story.read}</p><h2 className="mt-3 font-display text-3xl leading-none tracking-[-.04em] group-hover:text-primary">{story.title}</h2><p className="mt-4 text-sm leading-6 text-muted-foreground">{story.excerpt}</p><span className="mt-5 inline-flex items-center gap-2 text-sm font-bold">Read recipe <ArrowUpRight size={15} /></span></Link>)}</div>{visibleCount < stories.length && <button type="button" onClick={() => setVisibleCount(count => Math.min(count + 6, stories.length))} className="mx-auto mt-16 block rounded-full border border-primary px-6 py-3 text-sm font-bold text-primary hover:bg-primary hover:text-primary-foreground" data-testid="button-load-more-blog">Load more recipes</button>}</section></Shell>;
}

function BlogArticle() {
  const { slug } = useParams<{ slug: string }>();
  const story = stories.find(item => item.id === slug) ?? stories[0];
  return <Shell><Meta title={story.title} description={story.excerpt} keywords={story.keywords} image={story.image} type="article" /><article><header className="mx-auto max-w-4xl px-5 pb-12 pt-16 text-center lg:px-8 lg:pt-24"><Link href="/blog" className="font-mono-brand text-[10px] uppercase tracking-[.2em] text-primary" data-testid="link-back-blog">← Back to blog</Link><p className="mt-10 font-mono-brand text-[10px] uppercase tracking-[.2em] text-secondary">{story.category} · {story.read}</p><h1 className="mt-5 font-display text-5xl leading-[.92] tracking-[-.07em] md:text-7xl">{story.title}</h1><p className="mx-auto mt-7 max-w-xl text-lg leading-7 text-muted-foreground">{story.excerpt}</p></header><div className="mx-auto max-w-5xl px-5 lg:px-8"><PhotoArt src={story.image} alt={story.alt} label={story.category} className="min-h-[300px] md:min-h-[480px]" /></div><div className="prose prose-stone mx-auto max-w-2xl px-5 py-16 prose-headings:font-display prose-headings:font-semibold prose-p:leading-8 lg:px-8"><h2>Restaurant-style method</h2>{story.body.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}<blockquote>The first bite should make you look up from the table.</blockquote><p className="font-mono-brand text-xs uppercase tracking-[.16em] text-muted-foreground">Kitchen note · Sardaar JI Dhaba</p></div></article></Shell>;
}
...
function Legal({ terms = false }: { terms?: boolean }) {
  ...
  return <Shell><Meta title={title} /><PageIntro eyebrow="The fine print" title={title} copy="A clear, plain-language placeholder for the policies that help our guests and partners feel looked after." /><section className="mx-auto max-w-3xl px-5 pb-28 lg:px-8"><div className="space-y-10 border-t border-foreground/15 pt-8 text-sm leading-7 text-muted-foreground"><div><h2 className="font-display text-3xl text-foreground">This page is a placeholder</h2><p className="mt-3">Final {terms ? 'terms and conditions' : 'privacy policy'} content will be provided by the Sardaar JI team and legal advisors before launch.</p></div>{['What this covers', 'Information and communication', 'Your choices', 'Contact'].map((heading, index) => <div key={heading}><h2 className="font-display text-2xl text-foreground">{heading}</h2><p className="mt-3">Details for this section are to be provided. We believe in being straightforward about how information is collected, how services work, and how to reach the people behind the dhaba.</p>{index === 3 && <a href={`mailto:${brand.email}`} className="mt-3 inline-block font-bold text-primary underline" data-testid={`link-legal-email-${index}`}>{brand.email}</a>}</div>)}</div></section></Shell>;
}
