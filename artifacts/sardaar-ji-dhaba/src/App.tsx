...
function Legal({ terms = false }: { terms?: boolean }) {
  ...
  return <Shell><Meta title={title} /><PageIntro eyebrow="The fine print" title={title} copy="A clear, plain-language overview of how Sardaar JI Dhaba handles guest information, service communication and partner enquiries." /><section className="mx-auto max-w-3xl px-5 pb-28 lg:px-8"><div className="space-y-10 border-t border-foreground/15 pt-8 text-sm leading-7 text-muted-foreground"><div><h2 className="font-display text-3xl text-foreground">How we handle guest information</h2><p className="mt-3">Sardaar JI Dhaba collects information you share through forms, reservations, order enquiries and direct contact so we can respond clearly, improve service and keep communication helpful and relevant.</p></div>{['What this covers', 'Information and communication', 'Your choices', 'Contact'].map((heading, index) => <div key={heading}><h2 className="font-display text-2xl text-foreground">{heading}</h2><p className="mt-3">We keep this information limited to what is needed for service, support and general communication. If you need to update details or ask a question, you can reach us directly at {brand.email}.</p>{index === 3 && <a href={`mailto:${brand.email}`} className="mt-3 inline-block font-bold text-primary underline" data-testid={`link-legal-email-${index}`}>{brand.email}</a>}</div>)}</div></section></Shell>;
}
